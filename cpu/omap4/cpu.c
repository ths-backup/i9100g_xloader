...
static void scale_vcore_omap4430(unsigned int rev)
{
...
#if defined(CONFIG_PMIC_TWL6032)
	/* vdd_core - SMPS 2 - OPP100 - 1.228V */
	omap_vc_bypass_send_value(TWL6032_SRI2C_SLAVE_ADDR,
				  TWL6032_SRI2C_REG_ADDR_SMPS2, 0x2A);
	/* to make 2ms delay */
	spin_delay(400000);
	...
	/* to make 2ms delay */
	spin_delay(400000);
	/* vdd_iva - SMPS 5 - OPP50 - 1.190V */
	omap_vc_bypass_send_value(TWL6032_SRI2C_SLAVE_ADDR,
				  TWL6032_SRI2C_REG_ADDR_SMPS5, 0x19);
	/* to make 2ms delay */
	spin_delay(400000);
#else
	...
	/* vdd_mpu - VCORE 1 - OPP100 - ES2+: 1.3674V */
	vsel = (rev == OMAP4430_ES1_0) ? 0x3B : 0x3A;
	omap_vc_bypass_send_value(TWL6030_SRI2C_SLAVE_ADDR,
				  TWL6030_SRI2C_REG_ADDR_VCORE1, vsel);

	/* vdd_iva - VCORE 2 - OPP50 - ES2+: 1.190V */
	vsel = (rev == OMAP4430_ES1_0) ? 0x31 : 0x19;
	omap_vc_bypass_send_value(TWL6030_SRI2C_SLAVE_ADDR,
...
static void scale_vcore_omap4460(unsigned int rev)
{
	...
	omap_vc_bypass_send_value(TWL6030_SRI2C_SLAVE_ADDR,
				  TWL6030_SRI2C_REG_ADDR_VCORE2, 0x19);
}
